...
The goal of the visualization is to be able to compare the energy consumption of different countries. (PLOT NUMBER COMPARE COUNTRIES) As we can see above, there is a huge difference in the energy consumption between countries, the idea is then to focus on the energy consumption per capita for comparison and get a meaningful visualization. It will then be possible to see how each country's energy consumption differs and which countries have a similar consumption. For example, maybe countries geographically close have a similar consumption because of weather and climate similarities, etc..
...
The overview of the project will be the European map, where the energy consumption will be presented clearly based on visualisation techniques, like coloring the countries depending on energy usage. Then the base interactive part will be able to press several countries and then display/compare their energy consumption in a separate graph. 
